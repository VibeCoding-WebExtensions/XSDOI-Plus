...
| 网页桌宠 | 可拖动的圆球小宠物，贴底走动/蹦跳，边缘吸附，可放入自定义图片并在裁剪器中拖动/缩放圆形选区（重开面板选区正确还原）；显隐与裁剪点「保存配置」后立即应用，由 popup 控制显隐；全局注入，所有页面均显示；拖到空中松手后遵循抛物线惯性落地；与鼠标尾迹联动——飞行/下落时碰到圆点尾迹反弹，碰到带状尾迹根据速度方向反弹或滑行（贴底散步时不受尾迹影响）；双击宠物弹出悬浮聊天窗口（API 设置、系统提示词在 popup「网页桌宠」面板中统一配置；支持 Markdown 渲染和 LaTeX 公式显示（$...$ / \(...\) 行内，$$...$$ / \[...\] 独立；若模型误用反引号 ` 包裹简短公式也会自动识别为数学并渲染）；输入框 Enter 换行、Ctrl+Enter 发送；AI 回复流式输出（打字机效果，实时显示思考过程；发送后显示步骤状态：🔄 正在连接模型 → ⏳ 等待模型响应 → 💭 思考中，出错则显示 ⚠️ 错误）；支持「思考过程」折叠块（summary 标签，流式时展开实时显示、结束后收起，重开会话从历史恢复；兼容 DeepSeek 的 reasoning_content 与 OpenAI 的 reasoning）；多会话管理——新建/切换/重命名会话（重命名在会话列表删除按钮旁），历史对话持久化存储；系统提示词可自定义编辑、一键恢复默认；居中卡片+毛玻璃遮罩，点击遮罩空白处或点 × 关闭） | popup「网页桌宠」 |
